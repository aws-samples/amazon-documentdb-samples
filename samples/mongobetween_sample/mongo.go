...
func (m *Mongo) RoundTrip(msg *Message, tags []string) (_ *Message, err error) {
    ...
    // m.log.Info("readPref:"+rpref.String())
	server, err := m.selectServer(requestCursorID, collection, transactionDetails,rpref)
	if err != nil {
		return nil, err
	}
    ...
    // m.log.Info("conn:"+conn.Address().String())
	defer func() {
		err := conn.Close()
		if err != nil {
...
func (m *Mongo) selectServer(requestCursorID int64, collection string, transDetails *TransactionDetails, rpref *readpref.ReadPref) (server driver.Server, err error) {
