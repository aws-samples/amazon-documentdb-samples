# Steps to run MongoBetween connection proxy with Amazon DocumentDB

## Introduction

### Why do we need a connection proxy for Amazon DocumentDB

 The maximum number of connections that DocumentDB currently supports is 30K per instance and this is a hard limit. Scenarios where we see customer hitting these limits are with applications that are deployed using containers and have connections are opened for a prolonged period of time. A common design solution to minimize the connections created on the database instances is to use a connection proxy  layer.In this article, we introduce an open source proxy solutions and provides steps to install and deploy this proxy along with required application changes.

#### What is Mongobetween?

Mongobetween is a lightweight MongoDB connection pooler written in Golang. It's primary function is to handle a large number of incoming connections, and multiplex them a across a smaller connection pool to one or more DocumentDB clusters.
...
2.Add ```mongobetween``` executable to path
...
3. Copy the file ```test_mongobetween.py``` to working directory. This file contains python code which spins off a number of processes, as provided by argument ```--mongo-clients-count```. Each process creates its own collection, inserts a 1000 documents to the primary instance in Amazon DocumentDB, as provided by argument ```--docdb-uri``` , and then reads the 1000 documents back from a replica.
...
```
## Run Python script with direct connection to DocumentDB cluster with 200 concurrent processes
...
* Wait for the follwing lines to be printed
...
* Check for succesfull processes
...
* Check the number of connections opened to the primary instance from your DocumentDB cluster from monitoring dashboard

![direct-200-primary](files/direct-200-primary.png)

* Check the number of connections opened to the replica instance from your DocumentDB cluster from monitoring dashboard

![direct-200-secondary](files/direct-200-secondary.png)

## Run Python script with direct connection to DocumentDB cluster with 900 concurrent processes
...
```
* Wait for the follwing line to be pinted
```process starting -  899
process starting -  898
process starting -  897
Completed execution of script. Checks logs directory for results.```
```
* Wait for atleast 20 seconds and then check for exceptions in log files
```
cat  "$(ls -drt logs/* | tail -1)" |grep "exception in mongo client"
```
...
These are the consequences  of the Amazon DocumentDB R6G.large instances rejecting  connections beyond its limit of 1700. By default each mongo client would open connections in increments of two upto a max of 100 as goverened by Mongo URI parameter.Each process in our code creates one mongo client - hence with 900 clients, the code is trying to open 1800 connections to the primary
...
## Run test script with mongobeetween connection pooling

1. Start mongobetween connection proxy process,with maximum 300 connections to the Amazon DocumentDB cluster
...
5. Run the test with 900  concurrent processes but the uri pointing to mongobetween proxy - note that the proxy uses port ```27016```
...
You should not notice any exceptions since mongobetween is acting as connection buffer between the application and the Amazon DocumentDB instances. As set by ```maxPoolSize``` parameter in the uri supplied to mongobetween, you will notice the only ~250 connections are open in the primary. But still all 900 processes from our code are successful.
...
Note: this dashboard is created for a cluster with topology of one primary and one replica instance. Change the content according to your cluster topology.

## Conclusion

If you can use mongobetween effectively with your containerised applications, this will let you handle unplanned scalling events gracefully without exceptions being generated in the application. Please note that whenever you are sending more connection to mongobetween than what is established to the DocumentDB cluster from mongobetwwen, you will notice increased latencies in the application - this is trade off which you need to consider in your application design, timeout settings and exception handling.
